...
def loadParams(params):
    ...
    if os.path.exists(params) and os.path.isfile(params):
        ...
        dist_co = [float(f.readline()), float(f.readline()),
                   float(f.readline()), float(f.readline())]
...
def initVideoCapture(fileName):
    ...
    if cap is None or not cap.isOpened():
        ...
        image_shape = (int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)),
...
def getImagePoints():
    ret, corners = cv2.findChessboardCorners(
        currFrame, (3, 3), flags=cv2.CALIB_CB_FAST_CHECK)
    if ret:
...
def getImagePoints2():
    four_corners_img = ((600.0, 400.0), (620.0, 400.0),
                        (600.0, 420.0), (620.0, 420.0))
...
def display():
    ...
    # currFrame=cv2.imread("QR-Patron_pequenito.png")

    # currFrame=cv2.imread("Patron_mas_QRs.png");
    # currFrame=cv2.imread("QR-Patron.png")
    # currFrame=cv2.imread("QR-Patron_peque.png")

    # currFrame=cv2.imread("QR-Patron_pequeninito.png")
    ...
    if currFrame is not None:
        ...
        glDrawPixels(
            flippedImage.shape[1], flippedImage.shape[0], GL_BGR, GL_UNSIGNED_BYTE, flippedImage)
        ...
        glFrustum(-principalX / fx, (width - principalX) / fy,
                  (principalY - height) / fy, principalY / fy, near, far)
        ...
        ret, corners = cv2.findChessboardCorners(
            currFrame, (3, 3), flags=cv2.CALIB_CB_FAST_CHECK)
        #ret, corners = cv2.findChessboardCorners(currFrame, (4, 3), flags=cv2.CALIB_CB_FAST_CHECK)

        #print ("\t\tEsquinas ",corners.shape)
        ...
        if ret is True:
            ...
            print("Obj", objp)
            ret, rotv, tvecs = cv2.solvePnP(
                objp, corners2, cameraMatrix, dist_co, None, None, 0, cv2.SOLVEPNP_ITERATIVE)

            projectedImgPts, _ = cv2.projectPoints(
                objp, rotv, tvecs, cameraMatrix, dist_co)
            j = 0
            ...
            glDisable(GL_DEPTH_TEST)
            glDrawPixels(currFrame.shape[1], currFrame.shape[0],
                         GL_BGR, GL_UNSIGNED_BYTE, currFrame.data)
            glEnable(GL_DEPTH_TEST)
